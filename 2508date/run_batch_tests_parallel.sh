...
declare -a TEST_CASES=(
    "case1_default"
    "case2_samos"
    "case3_affiliatedordering"
    "case4_seperratedordering"
    "case5_COMBO1"
    "case6_COMBO2"
    "case7_FireAdvance"
    "case8_BinarySwitch"
    "case9_MOSAIC1"
    "case10_MOSAIC2"
    "case11_LSBSaturation"
    "case12_LSBSaturationAffilaited"
)
...
compile_config() {
    ...
    # Disable all NoC size configurations
    ...
    # Disable LLM mode for CNN tests
    sed -i 's|^#define AUTHORLLMSwitchON|//#define AUTHORLLMSwitchON|' "$COMPILE_DIR/src/parameters.hpp"
    # Disable all case configurations
    sed -i 's|^#define case[0-9][0-9]*_[a-zA-Z]*|//&|g' "$COMPILE_DIR/src/parameters.hpp"
...
TOTAL_RUNS=$(tail -n +2 "$SUMMARY_FILE" 2>/dev/null | wc -l)
FAILED_COUNT=$(grep -c "COMPILE_ERROR" "$SUMMARY_FILE" 2>/dev/null || echo "0")
...
echo "Runtime Summary (sorted by runtime):"
tail -n +2 "$SUMMARY_FILE" | grep -v "COMPILE_ERROR" | sort -t',' -k3 -n | while IFS=',' read -r noc test runtime; do
    printf "    %s / %s : %ss\n" "$noc" "$test" "$runtime"
done
echo "==========================================="
