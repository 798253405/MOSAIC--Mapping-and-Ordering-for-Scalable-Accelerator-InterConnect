...
def create_dataframe():
    ...
    data = {
        ...
        'Strategy': ['baseline', 'samos', 'affiliated', 'separated', 'mosaic-1', 'mosaic-2'] * 5,
        'Total_Cycles': [
            # 2_4x4
...
def create_comprehensive_analysis():
    ...
    for i, strategy in enumerate(strategies):
        ...
        # Add percentage reduction labels for SAMOS only
        if strategy == 'SAMOS':
            baseline_values = []
            for noc in noc_order:
            ...
            for j, (samos_val, baseline_val) in enumerate(zip(values, baseline_values)):
                if baseline_val > 0:
                    reduction = ((baseline_val - samos_val) / baseline_val) * 100
                    ax3.text(x_base[j] + i * bar_width, samos_val + 0.1,
                             f'-{reduction:.1f}%',
    ...
    # Highlight SAMOS effectiveness
    ax4.axvspan(-0.5, 2.5, alpha=0.3, color='yellow')
    ax4.text(1, -3, 'SAMOS Effective Range ≤8×8 NoC\n(for 128-token LLM configuration)', ha='center', fontsize=10,
    ...
    # Compare Baseline, SAMOS, and MOSAIC-2 (bars) + Separated (line)
    ...
    for noc in noc_order:
        ...
        samos_cycles.append(noc_data[noc_data['Strategy'] == 'SAMOS']['Total_Cycles'].values[0])
        ...
        samos_bittrans.append(noc_data[noc_data['Strategy'] == 'SAMOS']['BitTransitions'].values[0])
    ...
    bars2 = ax5.bar(x_positions, samos_cycles, bar_width_e,
    ...
    # Add SAMOS cycle reduction percentages
    for i, (baseline_c, samos_c) in enumerate(zip(baseline_cycles, samos_cycles)):
        if baseline_c != samos_c:
            ...
            if reduction > 0:
                ...
                ax5.text(x_positions[i], samos_c + y_offset,
                         f'-{reduction:.1f}%',
                         ha='center', va='bottom',
    ...
    samos_reduction = [(b - s) / b * 100 for b, s in zip(baseline_bittrans, samos_bittrans)]
    ...
    # Plot all three power reduction lines - ORDER: SAMOS, Separated, MOSAIC-2
    line1 = ax5_twin.plot(x_positions, samos_reduction, 'o-', linewidth=2.5,
                          markersize=9, color=colors['SAMOS'], label='SAMOS BT↓', alpha=0.9)
    line2 = ax5_twin.plot(x_positions, separated_reduction, '^-', linewidth=2.5,
                          markersize=9, color=colors['Separated'], label='Separated BT↓', alpha=0.9)
    line3 = ax5_twin.plot(x_positions, mosaic2_reduction, 's-', linewidth=2.5,
                          markersize=9, color=colors['MOSAIC-2'], label='MOSAIC-2 BT↓', alpha=0.9)

    # Key findings text box
    textstr = '''Key Finding (128-token):
Separated: ~31% power reduction
(pure power optimization)

SAMOS/MOSAIC-2: Balance both
performance and power metrics'''
    props = dict(boxstyle='round', facecolor='lightyellow', alpha=0.8)
    ax5.text(0.68, 0.50, textstr, transform=ax5.transAxes, fontsize=8,
    ...
    ax5.legend(handles1 + handles2, labels1 + labels2, loc='upper right', fontsize=9, ncol=1)
